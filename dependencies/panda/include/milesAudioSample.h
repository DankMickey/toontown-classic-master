...
public:
  ...
  virtual void query_channel_level_array(S32 l_amount);
  virtual float get_channel_level_array();
  virtual int get_int_channel_level_array();
  virtual S32 get_channel_level_amount();
  
  // 7.2a exclusively needed functions.
  virtual void set_downmix_dest_list(MSS_SPEAKER dest_list);
  virtual MSS_SPEAKER get_downmix_dest_list() const;
  virtual void set_downmix_source_list(MSS_SPEAKER source_list);
  virtual MSS_SPEAKER get_downmix_source_list() const;
  virtual void query_downmix_lists(int n_levels);
...
private:
  ...
  S32 numLevels;
  MSS_SPEAKER downmix_dest_list;
  MSS_SPEAKER downmix_source_list;
  MSS_SPEAKER new_downmix_dest_list;
  MSS_SPEAKER new_downmix_source_list;
  MSS_SPEAKER dest_list;
  MSS_SPEAKER source_list;
  float level_array;
  int int_level_array;
  float basic_level_array;
  int basic_int_level_array;
