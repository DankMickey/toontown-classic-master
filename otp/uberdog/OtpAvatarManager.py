...
class OtpAvatarManager(DistributedObject.DistributedObject):
    ...
    def avatarListResponse(self, pickleData):
        avatars = [] # TODO: FINISHME!!! #loads(pickleData)

        # TODO: UNPACK AVATAR!
        self.cr.loginFSM.request('chooseAvatar', [avatars])
    ...
    def sendCheckNameTyped(self, avId, nameSet):
        self.sendUpdate('sendCheckNameTyped', [
            str(avId), # TODO: Read the comment in the dclass files.
            nameSet])

    def sendCheckNameTypedResp(self, avId, pendingname, approvedname, rejectedname):
        namelist = [pendingname, approvedname, rejectedname]

        messenger.send('nameTypedAnswer',[
            int(avId),
            namelist])

    def sendRequestCreateAvatar(self, avDNA, avName, avPosition):
        self.sendUpdate('sendRequestCreateAvatar', [
            avDNA,
            avName,
            avPosition])

    def sendRequestCreateAvatarResp(self, avId):
        messenger.send('nameShopCreateAvatarDone', [
            avId])

    def requestChooseAvatar(self, avId):
        self.sendUpdate('requestChooseAvatar', [
            avId])