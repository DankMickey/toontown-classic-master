...
class OTPClientRepository(ClientRepositoryBase):
    ...
    def __init__(self, serverVersion, launcher = None, playGame = None):
        ...
        self.wantMagicWords = base.config.GetString('want-magic-words', 'True')
        ...
        if self.http.getVerifySsl():
            self.http.setClientCertificateFilename('resources/phase_3/etc/gameserver.txt') # TODO: FIXME!
            self.http.loadClientCertificate()
    ...
    def _handleConnected(self):
        self.handler = self._handleConnectedResp
    ...
    def _handleConnectedResp(self, msgType, di):
    ...
    def sendCreateAvatarMsg(self, avDNA, avName, avPosition):
        self.avatarMgr.sendRequestCreateAvatar(avDNA.makeNetString(), 
            avName, avPosition)
    ...
    def isPaid(self):
        paidStatus = base.config.GetString('force-paid-status', 'FULL')
        if not paidStatus:
