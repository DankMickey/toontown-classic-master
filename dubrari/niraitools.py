...
PYTHON_ROOT = os.path.join(NIRAI_ROOT, 'dependencies\panda\python')
PANDA3D_ROOT = os.path.join(NIRAI_ROOT, 'dependencies\panda')
THIRDPARTY_ROOT = os.path.join(NIRAI_ROOT, 'dependencies\thirdparty')
...
class NiraiCompilerBase:
    ...
    def add_nirai_files(self):
        for filename in ('src\aes.cxx', 'src\TTC.cxx'):
...
class NiraiPackager: 
    ...
    def __read_file(self, filename, mangler=None):
        ...
        try:
            ...
            print('WARNING: Failed to compile %s' % filename)
            return '', ('', 0)
    ...
    def add_panda3d_dirs(self):
        ...
        self.add_directory(os.path.join(PANDA3D_ROOT, 'direct'), mangler=_mangler)
        self.add_directory(os.path.join(PANDA3D_ROOT, 'pandac'), mangler=_mangler)
        self.add_directory(os.path.join(PANDA3D_ROOT, 'panda3d'), mangler=_mangler)
