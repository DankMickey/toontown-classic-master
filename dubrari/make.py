...
parser.add_argument('--compile-cxx', '-c', action='store_true',
                    help='Compile the CXX codes and generate TTCEngine.exe into built.')
...
# Compile the engine
if args.compile_cxx:
    compiler = NiraiCompiler('TTCEngine.exe')

    compiler.add_nirai_files()
    compiler.add_source('src/TTC.cxx')

    compiler.run()

# Compile the game data
if args.make_nri:
    pkg = TT2Packager('built/gamedata.bin')
