...
class DistributedCannonGame(DistributedMinigame):
    ...
    def _DistributedCannonGame__stopIntro(self):
        ...
        try:
         ...
         if self.introCameraInterval:
          self.introCameraInterval.finish()
        except:
         if self.introCameraInterval:
          self.introCameraInterval.finish()
    ...
    def _DistributedCannonGame__spawnCameraLookAtLerp(self, targetPos, targetLookAt, duration):
        ...
        targetSet = Quat()
        targetSet.setHpr(camera.getHpr())
        camera.setPos(oldPos)
        camera.setHpr(oldHpr)
        if self.introCameraInterval:
            self.introCameraInterval.finish()
        self.introCameraInterval = camera.posQuatInterval(duration, Point3(targetPos), targetSet, blendType='easeInOut')
        self.introCameraInterval.start()
