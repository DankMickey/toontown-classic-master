...
class SuitPlannerInteriorAI:
    ...
    def _genSuitInfos(self, numFloors, bldgLevel, bldgTrack):
        ...
        for currFloor in range(numFloors):
            infoDict = { }
            lvls = self._SuitPlannerInteriorAI__genLevelList(bldgLevel, currFloor, numFloors)
            activeDicts = []
            ...
            joinChances = self._SuitPlannerInteriorAI__genJoinChances(numReserve)
    ...
    def _SuitPlannerInteriorAI__genLevelList(self, bldgLevel, currFloor, numFloors):
        bldgInfo = SuitBuildingGlobals.SuitBuildingInfo[bldgLevel]
    ...
    def _SuitPlannerInteriorAI__setupSuitInfo(self, suit, bldgTrack, suitLevel, suitType):
    ...
    def _SuitPlannerInteriorAI__genSuitObject(self, suitZone, suitType, bldgTrack, suitLevel, revives = 0):
        newSuit = DistributedSuitAI.DistributedSuitAI(simbase.air, None)
        skel = self._SuitPlannerInteriorAI__setupSuitInfo(newSuit, bldgTrack, suitLevel, suitType)
        if skel:
    ...
    def genFloorSuits(self, floor):
        ...
        for activeSuitInfo in floorInfo['activeSuits']:
            suit = self._SuitPlannerInteriorAI__genSuitObject(self.zoneId, activeSuitInfo['type'], activeSuitInfo['track'], activeSuitInfo['level'], activeSuitInfo['revives'])
            activeSuits.append(suit)
        
        suitHandles['activeSuits'] = activeSuits
        reserveSuits = []
        for reserveSuitInfo in floorInfo['reserveSuits']:
            suit = self._SuitPlannerInteriorAI__genSuitObject(self.zoneId, reserveSuitInfo['type'], reserveSuitInfo['track'], reserveSuitInfo['level'], reserveSuitInfo['revives'])
