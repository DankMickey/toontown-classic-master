...
class DistributedBoardingPartyAI(DistributedObjectAI.DistributedObjectAI, BoardingPartyBase.BoardingPartyBase):
    ...
    def addWacthAvStatus(self, avId):
        self.acceptOnce(self.air.getAvatarExitEvent(avId), self.handleAvatarDisco, extraArgs = [
            avId])
        self.accept(self.staticGetLogicalZoneChangeEvent(avId), self.handleAvatarZoneChange, extraArgs = [
            avId])
    ...
    def requestInvite(self, inviteeId):
                ...
                if len(self.getGroupMemberList(leaderId)) >= self.maxSize:
                    ...
                    for memberId in groupList[0]:
                        if not memberId == inviterId:
                            self.sendUpdateToAvatarId(memberId, 'postMessageInvited', [
                                inviteeId,
                                inviterId])
                            continue
        ...
        elif self.avIdDict.has_key(inviteeId):
    ...
    def requestGoToSecondTime(self, elevatorId):
        ...
        if self.testGoButtonRequirements(callerId, elevatorId):
            ...
            THREE_SECONDS = 3.0
            taskMgr.doMethodLater(THREE_SECONDS, self.sendAvatarsToDestinationTask, self.uniqueName('sendAvatarsToDestinationTask'), extraArgs = [
                elevatorId,
                avList], appendTask = True)
