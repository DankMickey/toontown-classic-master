...
class DistributedPartyCannonActivity(DistributedPartyActivity):
    ...
    def load(self):
        ...
        self.accept(FireworksStartedEvent, self._DistributedPartyCannonActivity__handleFireworksStarted)
        self.accept(FireworksFinishedEvent, self._DistributedPartyCannonActivity__handleFireworksFinished)
    ...
    def unload(self):
        ...
        if self.localFlyingToon:
            self._DistributedPartyCannonActivity__resetToon(self.localFlyingToon)
    ...
    def disable(self):
        ...
        if self.localFlyingToonId:
            self._DistributedPartyCannonActivity__stopCollisionHandler(self.localFlyingToon)
            self._DistributedPartyCannonActivity__stopLocalFlyTask(self.localFlyingToonId)
    ...
    def _DistributedPartyCannonActivity__fireCannonTask(self, task):
        ...
        if self.isLocalToonId(toonId):
            self.inWater = 0
            flightResults = self._DistributedPartyCannonActivity__calcFlightResults(cannon, toonId, launchTime)
            if not isClient():
        ...
        shootTask = Task(self._DistributedPartyCannonActivity__shootTask, self.taskNameShoot)
        shootTask.info = {
            'toonId': toonId,
            'cannon': cannon }
        if self.isLocalToonId(toonId):
            ...
            flyTask = Task(self._DistributedPartyCannonActivity__localFlyTask, self.taskNameFly)
            flyTask.info = info
    ...
    def setToonTrajectory(self, avId, launchTime, x, y, z, h, p, r, vx, vy, vz):
        ...
        startT = globalClockDelta.networkToLocalTime(launchTime, bits = 31) + 0.20000000000000001
    ...
    def _DistributedPartyCannonActivity__calcFlightResults(self, cannon, toonId, launchTime):
    ...
    def landToon(self, toonId):
        ...
        if toon is not None:
            ...
            if toon == base.localAvatar:
                self._DistributedPartyCannonActivity__stopCollisionHandler(base.localAvatar)
    ...
    def _DistributedPartyCannonActivity__updateFlightVelocity(self, trajectory):
    ...
    def _DistributedPartyCannonActivity__localFlyTask(self, task):
        toon = task.info['toon']
        if toon.isEmpty():
            self._DistributedPartyCannonActivity__resetToonToCannon(self.localFlyingToon)
            return Task.done
        ...
        if self.hitBumper:
            ...
            self._DistributedPartyCannonActivity__updateFlightVelocity(task.info['trajectory'])
        ...
        self._DistributedPartyCannonActivity__setFlyingCameraView(task.info['toon'], cameraView, deltaT)
        return Task.cont

    
    def _DistributedPartyCannonActivity__setFlyingCameraView(self, toon, view, deltaT):
        if toon != base.localAvatar:
            return None
        ...
        if view == 0:
            ...
            relativeSpeed = math.pow(vec.length() / 60.0, 2) + 0.10000000000000001
    ...
    def _checkFlyingToonCollision(self, task = None):
        ...
        if self.handler.getNumEntries():
            ...
            if activeEntry is not None:
                self._DistributedPartyCannonActivity__handleFlyingToonCollision(activeEntry)
    ...
    def _DistributedPartyCannonActivity__handleFlyingToonCollision(self, collisionEntry):
        ...
        if hitNode.find('cSphere') == 0 and hitNode.find('treasureSphere') == 0 and hitNode.find('prop') == 0 and hitNode.find('distAvatarCollNode') == 0 and hitNode.find('CannonSphere') == 0 and hitNode.find('plotSphere') == 0 and hitNode.find('flySphere') == 0 and hitNode.find('FishingSpotSphere') == 0 and hitNode.find('TrampolineTrigger') == 0 and hitNode == 'gagtree_collision' and hitNode == 'sign_collision' and hitNode == 'FlowerSellBox' or hitPylonBelowWater:
        ...
        bumperNodes = [
            'sky_collision'] + PartyCannonCollisions['bounce'] + PartyCannonCollisions['fence']
        cloudBumpers = PartyCannonCollisions['clouds']
        bumperNodes += cloudBumpers
        if hitNode in bumperNodes and hitNode.find('cogPie') == 0 or PartyCannonCollisions['trampoline_bounce'] in hitNode:
            if hitNode == 'sky_collision' and hitNode in PartyCannonCollisions['fence'] or hitNode.find('cogPie') == 0:
                self._DistributedPartyCannonActivity__hitFence(self.localFlyingToon, collisionEntry)
                ...
                self.hitCloud = 1
                self._DistributedPartyCannonActivity__hitBumper(self.localFlyingToon, collisionEntry, hitSound, kr = 0.089999999999999997, angVel = 5)
                self.hitBumper = 0
            elif hitNode in cloudBumpers:
                self._DistributedPartyCannonActivity__hitCloudPlatform(self.localFlyingToon, collisionEntry)
            elif hitNode == 'statuaryCol':
                self._DistributedPartyCannonActivity__hitStatuary(self.localFlyingToon, collisionEntry)
            else:
        ...
        if hitNode in PartyCannonCollisions['ground']:
            track.append(Func(self._DistributedPartyCannonActivity__hitGround, self.localFlyingToon, pos))
            track.append(Wait(1.0))
            track.append(Func(self._DistributedPartyCannonActivity__setToonUpright, self.localFlyingToon, self.landingPos))
        elif hitNode in PartyCannonCollisions['fence']:
            track.append(Func(self._DistributedPartyCannonActivity__hitFence, self.localFlyingToon, collisionEntry))
        elif hitNode == 'collision3':
            track.append(Func(self._DistributedPartyCannonActivity__hitWater, self.localFlyingToon, pos, collisionEntry))
            track.append(Wait(2.0))
            track.append(Func(self._DistributedPartyCannonActivity__setToonUpright, self.localFlyingToon, self.landingPos))
        elif hitNode.find('cloudSphere') == 0:
            track.append(Func(self._DistributedPartyCannonActivity__hitCloudPlatform, self.localFlyingToon, collisionEntry))
        else:
    ...
    def _DistributedPartyCannonActivity__hitBumper(self, avatar, collisionEntry, sound, kr = 0.59999999999999998, angVel = 1):
        self.hitBumper = 1
        base.playSfx(sound)
    ...
    def _DistributedPartyCannonActivity__hitGround(self, avatar, pos, extraArgs = []):
    ...
    def _DistributedPartyCannonActivity__hitFence(self, avatar, collisionEntry, extraArgs = []):
        self.notify.debug('__hitFence')
        self._DistributedPartyCannonActivity__hitBumper(avatar, collisionEntry, self.sndHitHouse, kr = 0.20000000000000001, angVel = 3)

    
    def _DistributedPartyCannonActivity__hitWater(self, avatar, pos, collisionEntry, extraArgs = []):
        hitP = avatar.getPos(render)
        if hitP[2] > ToontownGlobals.EstateWakeWaterHeight:
            self.notify.debug('we hit the ground before we hit water')
            self._DistributedPartyCannonActivity__hitGround(avatar, pos, extraArgs)
            return None
    ...
    def _DistributedPartyCannonActivity__hitStatuary(self, avatar, collisionEntry, extraArgs = []):
        self._DistributedPartyCannonActivity__hitBumper(avatar, collisionEntry, self.sndHitHouse, kr = 0.40000000000000002, angVel = 5)
    ...
    def _DistributedPartyCannonActivity__hitCloudPlatform(self, avatar, collisionEntry, extraArgs = []):
        if not (self.hitBumper) and not (self.hitCloud):
            self.hitCloud = 1
            self._DistributedPartyCannonActivity__hitBumper(avatar, collisionEntry, self.sndBounce1, kr = 0.34999999999999998, angVel = 5)
            self.hitBumper = 0
            if self._lastCloudHit is None:
                ...
                self._DistributedPartyCannonActivity__playDustCloud(avatar, collisionEntry.getSurfacePoint(render))
                self.flyingToonCloudsHit += 1
                taskMgr.doMethodLater(0.25, self._DistributedPartyCannonActivity__reactivateLastCloudHit, DistributedPartyCannonActivity.REACTIVATE_CLOUD_TASK)
