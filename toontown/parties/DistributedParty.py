...
class DistributedParty(DistributedObject.DistributedObject):
    ...
    def setPartyInfoTuple(self, partyInfoTuple):
        ...
        if not PartyGlobals.ActivityIds.PartyJukebox in allActIds:
            pass
        ...
        if config.GetBool('show-debug-party-grid', 0):
            ...
            for i in range(len(self.grid)):
                for j in range(len(self.grid[i])):
                    ...
                    np.setScale(12)
                    np.setP(-90.0)
                    np.setPos(PartyUtils.convertDistanceFromPartyGrid(j, 0) - 6.0, PartyUtils.convertDistanceFromPartyGrid(i, 1) - 6.0, 0.10000000000000001)
    ...
    def getClearSquarePositions(self):
        ...
        for y in range(len(self.grid)):
            for x in range(len(self.grid[0])):
                if self.grid[y][x]:
                    pos = (PartyUtils.convertDistanceFromPartyGrid(x, 0), PartyUtils.convertDistanceFromPartyGrid(y, 1), 0.10000000000000001)
    ...
    def loadPartyCountdownTimer(self):
        ...
        self.partyClockModel.reparentTo(base.cr.playGame.hood.loader.geom)
        self.partyClockModel.find('**/frontText_locator').setY(-1.1000000000000001)
        self.partyClockModel.find('**/backText_locator').setY(0.63300000000000001)
    ...
    def getTimer(self, parent):
        ...
        timer = { }
        timer['minute'] = DirectLabel(parent = parent, pos = TTLocalizer.DPtimerMinutePos, relief = None, text = '59', text_align = TextNode.ACenter, text_font = timeFont, text_fg = (0.69999999999999996, 0.29999999999999999, 0.29999999999999999, 1.0), scale = TTLocalizer.DPtimerMinute)
        timer['colon'] = DirectLabel(parent = parent, pos = TTLocalizer.DPtimerColonPos, relief = None, text = ':', text_align = TextNode.ACenter, text_font = timeFont, text_fg = (0.69999999999999996, 0.29999999999999999, 0.29999999999999999, 1.0), scale = TTLocalizer.DPtimerColon)
        timer['second'] = DirectLabel(parent = parent, relief = None, pos = TTLocalizer.DPtimerSecondPos, text = '14', text_align = TextNode.ACenter, text_font = timeFont, text_fg = (0.69999999999999996, 0.29999999999999999, 0.29999999999999999, 1.0), scale = TTLocalizer.DPtimerSecond)
        timer['textLabel'] = DirectLabel(parent = parent, relief = None, pos = (0.0, 0.0, 1.1499999999999999), text = TTLocalizer.PartyCountdownClockText, text_font = timeFont, text_fg = (0.69999999999999996, 0.29999999999999999, 0.29999999999999999, 1.0), scale = TTLocalizer.DPtimerTextLabel)
        return timer
