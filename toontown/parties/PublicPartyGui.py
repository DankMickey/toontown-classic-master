...
class PublicPartyGui(DirectFrame):
    ...
    def load(self):
        ...
        self.titleLabel = DirectLabel(parent = self, relief = None, text = TTLocalizer.PartyGateTitle, pos = self.gui.find('**/title_locator').getPos(), scale = 0.10000000000000001)
        ...
        self.partyStartButton = DirectButton(parent = self, relief = None, text = TTLocalizer.PartyGateGoToParty, text_align = TextNode.ACenter, text_scale = TTLocalizer.PPGpartyStartButton, text_pos = (pos[0], pos[2]), geom = (self.gui.find('**/startButton_up'), self.gui.find('**/startButton_down'), self.gui.find('**/startButton_rollover'), self.gui.find('**/startButton_inactive')), command = self._startParty)
    ...
    def createListAndLabel(self, typeString, numItems):
        list = DirectScrolledList(parent = self, relief = None, incButton_image = (self.gui.find('**/%sButtonDown_up' % typeString), self.gui.find('**/%sButtonDown_down' % typeString), self.gui.find('**/%sButtonDown_rollover' % typeString), self.gui.find('**/%sButtonDown_inactive' % typeString)), incButton_relief = None, decButton_image = (self.gui.find('**/%sButtonUp_up' % typeString), self.gui.find('**/%sButtonUp_down' % typeString), self.gui.find('**/%sButtonUp_rollover' % typeString), self.gui.find('**/%sButtonUp_inactive' % typeString)), decButton_relief = None, itemFrame_pos = self.gui.find('**/%s_locator' % typeString).getPos(), itemFrame_relief = None, numItemsVisible = numItems, forceHeight = 0.055)
        strings = {
            'activities': TTLocalizer.EventsPageHostingTabActivityListTitle,
            'parties': TTLocalizer.PartyGatePartiesListTitle }
        label = DirectLabel(parent = self, relief = None, text = strings[typeString], text_scale = 0.059999999999999998, pos = self.gui.find('**/%sText_locator' % typeString).getPos())
        return (list, label)
    ...
    def refresh(self, partyInfoTupleList):
        ...
        for (index, partyTuple) in enumerate(sortedList):
            ...
            item = DirectButton(relief = DGG.RIDGE, borderWidth = (0.01, 0.01), frameSize = (-0.01, 0.45000000000000001, -0.014999999999999999, 0.040000000000000001), frameColor = self.normalFrameColor, text = hostName, text_align = TextNode.ALeft, text_bg = Vec4(0.0, 0.0, 0.0, 0.0), text_scale = 0.044999999999999998, command = self.partyClicked)
            ...
            num = DirectLabel(relief = DGG.RIDGE, borderWidth = (0.01, 0.01), frameSize = (0.0, otherInfoWidth, -0.014999999999999999, 0.040000000000000001), frameColor = self.normalFrameColor, text = '%d' % numberOfGuests, text_align = TextNode.ALeft, text_scale = 0.044999999999999998, text_pos = (0.01, 0, 0), pos = (0.45000000000000001, 0.0, 0.0))
            num.reparentTo(item)
            item.numLabel = num
            actLabelPos = num.getPos()
            actLabelPos.setX(actLabelPos.getX() + otherInfoWidth)
            actLabel = DirectLabel(relief = DGG.RIDGE, borderWidth = (0.01, 0.01), frameSize = (0.0, otherInfoWidth, -0.014999999999999999, 0.040000000000000001), frameColor = self.normalFrameColor, text = '%d' % numActivities, text_align = TextNode.ALeft, text_scale = 0.044999999999999998, text_pos = (0.01, 0, 0), pos = actLabelPos)
            ...
            minLabel = DirectLabel(relief = DGG.RIDGE, borderWidth = (0.01, 0.01), frameSize = (0.0, otherInfoWidth, -0.014999999999999999, 0.040000000000000001), frameColor = self.normalFrameColor, text = '%d' % minLeft, text_align = TextNode.ALeft, text_scale = 0.044999999999999998, text_pos = (0.01, 0, 0), pos = minLabelPos)
    ...
    def fillActivityList(self, activityIds):
        ...
        for activityId in sortedList:
            ...
            item = DirectLabel(relief = None, text = text, text_align = TextNode.ACenter, text_scale = 0.050000000000000003, text_pos = (0.0, -0.14999999999999999), geom_scale = 0.29999999999999999, geom_pos = Vec3(0.0, 0.0, 0.070000000000000007), geom = PartyUtils.getPartyActivityIcon(self.activityIconsModel, PartyGlobals.ActivityIds.getString(activityId)))
    ...
    def createPartyListAndLabel(self, typeString, numItems):
        ...
        label = DirectLabel(parent = self, text_align = TextNode.ALeft, relief = None, text = strings[typeString], text_scale = 0.059999999999999998, pos = hostPos)
        curPos = label.getPos()
