...
class DistributedToonAI(DistributedPlayerAI.DistributedPlayerAI, DistributedSmoothNodeAI.DistributedSmoothNodeAI, PetLookerAI.PetLookerAI):
    ...
    def deleteDummy(self):
        if __debug__:
         self.air.notify.debug('----deleteDummy DistributedToonAI %d ' % self.doId)
        if self.inventory:
            self.inventory.unload()
    ...
    def ban(self, comment):
        self.air.notify.warning('Banned Toon ' + str(self.doId) + ", " + str(self.DISLid) + " for " + str(comment) + "!")
    ...
    def setDNAString(self, string):
        ...
        if simbase.config.GetBool('adjust-dna', True) and self.verifyDNA() == False:
            ...
            self.air.notify.warning(logStr)
    ...
    def setInventory(self, inventoryNetString):
        ...
        if lengthMatch != 0:
            ...
            if oldTracks == 0 and oldLevels == 0:
                self.air.writeServerEvent('suspicious', self.doId, " 'Reseting invalid inventory to MAX on toon'")
    ...
    def setDefaultShard(self, shard):
        ...
        if __debug__:
         self.air.notify.debug('setting default shard to %s' % shard)
        
    def d_setDefaultShard(self, shard):
        self.sendUpdate('setDefaultShard', [shard])
        
    def b_setDefaultShard(self, shard):
        self.setDefaultShard(shard)
        self.d_setDefaultShard(shard)
    ...
    def setDefaultZone(self, zone):
        ...
        if __debug__:
         self.air.notify.debug('setting default zone to %s' % zone)
    ...
    def setShtickerBook(self, string):
        if __debug__:
         self.air.notify.debug('setting shticker book to %s' % string)
    ...
    def setFriendsList(self, friendsList):
        if __debug__:
         self.air.notify.debug('setting friends list to %s' % self.friendsList)
    ...
    def setMaxNPCFriends(self, max):
        ...
        if self.maxNPCFriends != 8 and self.maxNPCFriends != 16:
            self.air.writeServerEvent('suspicious', self.doId, " 'Wrong max SOS cards: ", self.maxNPCFriends, "'")
    ...
    def attemptAddNPCFriend(self, npcFriend, numCalls = 1):
        if numCalls <= 0:
            self.air.writeServerEvent('suspicious', self.doId, " 'invalid numCalls: '", numCalls, "'")
            return 0
        ...
        if self.NPCFriendsDict.has_key(npcFriend):
            ...
            self.air.writeServerEvent('suspicious', self.doId, " 'invalid NPC: '", npcFriend, "'")
            return 0
    ...
    def attemptSubtractNPCFriend(self, npcFriend):
        if not self.NPCFriendsDict.has_key(npcFriend):
            self.air.notify.warning('attemptSubtractNPCFriend: invalid NPC %s' % npcFriend)
            return 0
    ...
    def removeItemInAccessoriesList(self, accessoryType, geomIdx, texIdx, colorIdx):
        ...
        if listLen < 3:
            self.air.writeServerEvent('suspicious', self.doId, " 'Accessory list is not long enough to delete anything'")
            return 0
    ...
    def b_setAnimState(self, animName, animMultiplier):
        if animName in ToontownGlobals.ProtectedAnimList:
         self.air.notify.warning('DistributedToon: Toon '+ str(self.doId) +" used a protected animation named " + str(animName) + "!")
    ...
    def setCogRadar(self, radar):
        if not radar:
            logStr = ('cogRadar set to bad value: %s. Resetting to [0,0,0,0]' % radar)
            self.air.writeServerEvent('suspicious', self.doId, logStr)
            self.cogRadar = [
                0,
                0,
    ...
    def setBuildingRadar(self, radar):
        if not radar:
            logStr = ('buildingRadar set to bad value: %s. Resetting to [0,0,0,0]' % radar)
            self.air.writeServerEvent('suspicious', self.doId, logStr)
            self.buildingRadar = [
                0,
                0,
    ...
    def setCogTypes(self, types):
        if not types:
            logStr = ('cogTypes set to bad value: %s. Resetting to [0,0,0,0]' % types)
            self.air.writeServerEvent('suspicious', self.doId, logStr)
            self.cogTypes = [
                0,
                0,
    ...
    def setCogLevels(self, levels):
        if not levels:
            logStr = ('cogLevels set to bad value: %s. Resetting to [0,0,0,0]' % levels)
            self.air.writeServerEvent('suspicious', self.doId, logStr)
            self.cogLevels = [
                0,
                0,
    ...
    def getNumPromotions(self, dept):
        if dept not in SuitDNA.suitDepts:
            logStr = ('getNumPromotions: Invalid parameter dept=%s' % dept)
            self.air.writeServerEvent('suspicious', self.doId, logStr)
            return 0
    ...
    def setCogParts(self, parts):
        if not parts:
            logStr = ('cogParts set to bad value: %s. Resetting to [0,0,0,0]' % parts)
            self.air.writeServerEvent('suspicious', self.doId, logStr)
            self.cogParts = [
                0,
                0,
    ...
    def setCogMerits(self, merits):
        if not merits:
            logStr = ('cogMerits set to bad value: %s. Resetting to [0,0,0,0]' % merits)
            self.air.writeServerEvent('suspicious', self.doId, logStr)
            self.cogMerits = [
                0,
                0,
    ...
    def b_setZonesVisited(self, zonesVisitedArray):
        self.zonesVisited = zonesVisitedArray
        self.d_setZonesVisited(zonesVisitedArray)

    
    def d_setZonesVisited(self, zonesVisitedArray):
        self.sendUpdate('setZonesVisited', [
            zonesVisitedArray])
