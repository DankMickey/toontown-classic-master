...
class ShardHandle:
    ...
    def updateInvasion(self, name, skel, start, duration, numCogs):
        self.cogName = name
        self.skel = skel
        self.start = start
...
class ShardAPIManagerUD(DirectObject):
    ...
    def _doUpdateShard(self, dgi):
        ...
        if updateType == 'block':
            ...
            shard.updateInvasion(cogName, skel, start, duration, 0)
    ...
    def __handleInvasionEvent(self, data):
        ...
        shard = self.shards[shardId]
        shard.updateInvasion(data['suitName'], data['skel'], data['startTime'], data['duration'], data['numCogs'])
