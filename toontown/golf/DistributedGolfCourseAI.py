...
class DistributedGolfCourseAI(DistributedObjectAI.DistributedObjectAI, FSM):
    ...
    def enterWaitJoin(self):
        ...
        self._DistributedGolfCourseAI__barrier = ToonBarrier('waitClientsJoin', self.uniqueName('waitClientsJoin'), self.avIdList, JOIN_TIMEOUT, allAvatarsJoined, handleTimeout)
    ...
    def enterWaitReadyCourse(self):
        ...
        self._DistributedGolfCourseAI__barrier = ToonBarrier('WaitReadyCourse', self.uniqueName('WaitReadyCourse'), self.avIdList, READY_TIMEOUT, allAvatarsInCourse, handleTimeout)
    ...
    def enterWaitReadyHole(self):
        ...
        self._DistributedGolfCourseAI__barrier = ToonBarrier('WaitReadyHole', self.uniqueName('WaitReadyHole'), stillPlaying, READY_TIMEOUT, allAvatarsInHole, handleTimeout)
        for avId in self.avStateDict.keys():
            if self.avStateDict[avId] == ONHOLE:
                self._DistributedGolfCourseAI__barrier.clear(avId)
                continue
