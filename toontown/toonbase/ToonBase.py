...
from toontown.nametag.MarginManager import MarginManager
from toontown.settings.ToontownSettings import ToontownSettings
__builtin__.Settings = ToontownSettings()
...
class ToonBase(OTPBase.OTPBase):
    ...
    def __init__(self):
        ...
        self.accept('f2', self.printDoId2do)
        self.setupCullBins()

    def setupCullBins(self):
        self.binManager = CullBinManager.getGlobalPtr()
        self.binManager.addBin('gui-popup', CullBinManager.BTUnsorted, 60)
        self.binManager.addBin('shadow', CullBinManager.BTFixed, 15)
        self.binManager.addBin('ground', CullBinManager.BTFixed, 14)

    def printDoId2do(self):
    ...
    def disableShowbaseMouse(self):
        ...
        if self.mouseInterface:
            self.mouseInterface.reparentTo(hidden)
        
        if base.mouse2cam:
            self.mouse2cam.reparentTo(hidden)
    ...
    def startShow(self, cr, launcherServer = None):
        ...
        for name in gameServer.split(';'):
            url = URLSpec(name, 1)
            if base.config.GetBool('want-ssl-support'):
                url.setScheme('s')
            if not url.hasPort():
