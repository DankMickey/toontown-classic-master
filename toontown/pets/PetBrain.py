...
from toontown.pets.CPetBrain import CPetBrain
import random
import types
        ...
        if not self.inMovie:
            ...
            if tSinceLastLonelinessUpdate >= PetConstants.LonelinessUpdatePeriod:
                ...
                if numLookers:
                    ...
                    self.pet.lerpMood('loneliness', max(-1.0, dt * -0.0030000000000000001 * numLookers))
                    if numLookers > 5:
    ...
    def _handleActionObserve(self, observe):
        ...
        if action == OA.ATTENDED_START:
            ...
            if avId != self.pet.doId:
                ...
                PetBrain.notify.debug('%s.CHANGE_ZONE: %s, %s->%s' % (self.pet.doId, avId, oldZoneId, newZoneId))
