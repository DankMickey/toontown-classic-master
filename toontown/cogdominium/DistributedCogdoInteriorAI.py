...
class DistributedCogdoInteriorAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def _DistributedCogdoInteriorAI__addToon(self, toonId):
        ...
        self.accept(event, self._DistributedCogdoInteriorAI__handleUnexpectedExit, extraArgs = [
            toonId])
    ...
    def elevatorDone(self):
        ...
        if self._DistributedCogdoInteriorAI__allToonsResponded() and self.ignoreElevatorDone == 0:
    ...
    def reserveJoinDone(self):
        ...
        self.responses[toonId] += 1
        if self._DistributedCogdoInteriorAI__allToonsResponded() and self.ignoreReserveJoinDone == 0:
    ...
    def enterElevator(self):
        ...
        self.timer.startCallback(BattleBase.ELEVATOR_T + ElevatorData[ELEVATOR_NORMAL]['openTime'] + BattleBase.SERVER_BUFFER_TIME, self._DistributedCogdoInteriorAI__serverElevatorDone)
    ...
    def _DistributedCogdoInteriorAI__createFloorBattle(self):
        if self.isBossFloor(self.currentFloor):
            ...
            bossBattle = 0
        self.battle = DistributedBattleBldgAI.DistributedBattleBldgAI(self.air, self.zoneId, self._DistributedCogdoInteriorAI__handleRoundDone, self._DistributedCogdoInteriorAI__handleBattleDone, bossBattle = bossBattle)
    ...
    def _DistributedCogdoInteriorAI__handleRoundDone(self, toonIds, totalHp, deadSuits):
        totalMaxHp = 0
        for suit in self.suits:
    ...
    def _DistributedCogdoInteriorAI__handleBattleDone(self, zoneId, toonIds):
        if len(toonIds) == 0:
            taskName = self.taskName('deleteInterior')
            taskMgr.doMethodLater(10, self._DistributedCogdoInteriorAI__doDeleteInterior, taskName)
    ...
    def enterReservesJoining(self):
        self._DistributedCogdoInteriorAI__resetResponses()
        self.timer.startCallback(ElevatorData[ELEVATOR_NORMAL]['openTime'] + SUIT_HOLD_ELEVATOR_TIME + BattleBase.SERVER_BUFFER_TIME, self._DistributedCogdoInteriorAI__serverReserveJoinDone)
