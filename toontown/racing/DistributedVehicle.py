...
class DistributedVehicle(DistributedSmoothNode.DistributedSmoothNode, Kart.Kart, FSM.FSM):
    ...
    surfaceModifiers = {
        'asphalt': {
            ...
            'particleColor': Vec4(0.69999999999999996, 0.69999999999999996, 0.69999999999999996, 1.0) },
        'gravel': {
            'shake': 0.20000000000000001,
            'driftMin': 65,
            'windResistance': 0.20000000000000001,
            'particleColor': Vec4(0.53000000000000003, 0.53000000000000003, 0.53000000000000003, 1.0) },
        'dirt': {
        ...
        'grass': {
            ...
            'particleColor': Vec4(0.80000000000000004, 0.41999999999999998, 0.80000000000000004, 1.0) },
        'ice': {
            'shake': 0.0,
            'driftMin': 0,
            'windResistance': 0.01,
    ...
    def _DistributedVehicle__setupCollisions(self):
        ...
        self.accept('enterWallCollision', self._DistributedVehicle__wallCollisionStart)
        self.accept('exitWallCollision', self._DistributedVehicle__wallCollisionStop)
        ...
        self.accept('floorCollision', self._DistributedVehicle__groundCollision)
    ...
    def fireSparkParticles(self, side):
        ...
        if not spark.effect.isEnabled():
            ...
            spark.start()
            taskMgr.doMethodLater(0.25, self.stopSparkParticles, 'sparkTimer-' + side, extraArgs = [
                side])
    ...
    def enterControlled(self, avId):
        ...
        if self.localVehicle:
            ...
            self.updateNonLocalTask = taskMgr.add(self._DistributedVehicle__updateNonlocalVehicle, taskName)
    ...
    def _DistributedVehicle__createPieWindshield(self):
        self.piePieces = []
        for piece in range(self.numPieChunks):
            self.piePieces.append(DirectLabel(relief = None, pos = (0.0, 0.0, 0.0), image = self.pieSplatter, image_scale = (0.5, 0.5, 0.5), text = ' ', text_scale = 0.17999999999999999, text_fg = (1, 0, 1, 1), text_pos = (-0.0, 0.0, 0), text_font = ToontownGlobals.getSignFont(), textMayChange = 1))
    ...
    def showPieces(self):
        ...
        for piece in self.piePieces:
            piece.setPos(randFloat(-xRange, xRange), randFloat(-0.10000000000000001, 3.5), randFloat(-0.90000000000000002, 0.90000000000000002))
            piece.setScale(randFloat(1.1000000000000001, 2.0), 1, randFloat(1.1000000000000001, 2.0))
            piece.show()
    ...
    def splatPie(self):
        ...
        if self.pieCount == 0:
            self.pieCount = 15
            taskMgr.doMethodLater(1, self._DistributedVehicle__countPies, ' ', extraArgs = [])
            taskMgr.add(self._DistributedVehicle__slidePies, 'slidePies', priority = 25)
        else:
            self.pieCount = 15
    ...
    def _DistributedVehicle__enableControlInterface(self):
        ...
        taskMgr.add(self._DistributedVehicle__watchControls, 'watchVehicleControls', priority = 25)
        if not self.speedometer:
            ...
            self.speedometer = DirectLabel(relief = None, pos = (1.24, 0.0, -0.97999999999999998), text = str(0), text_scale = 0.17999999999999999, text_fg = bodyColor, text_pos = (-0.040000000000000001, 0.02, 0), text_font = ToontownGlobals.getSignFont())
        else:
    ...
    def startTurbo(self):
        ...
        self.cameraTrack = Sequence(Func(self.turboStartSfx.play), cameraZoomIn, Func(lambda : self.setTurbo(True)), Wait(turboDuration), Func(self._DistributedVehicle__stopTurbo), cameraToNormal)
        ...
        if self.localVehicle:
            if self.lifter.isOnGround():
                if self.offGround > 10:
                    ...
                    bumpDown1 = kart.posInterval(0.10000000000000001, Vec3(0, 0, -1))
                    bumpUp1 = kart.posInterval(0.14999999999999999, Vec3(0, 0, 0))
                    bumpDown2 = kart.posInterval(0.5, Vec3(0, 0, -0.40000000000000002))
                    bumpUp2 = kart.posInterval(0.69999999999999996, Vec3(0, 0, 0))
    ...
    def amIClampingPosition(self):
        if not self.getX() == -3276 and self.getX() == 3276 and self.getY() == -3276 and self.getY() == 3276 and self.getZ() == -3276:
            pass
    ...
    def _DistributedVehicle__watchControls(self, task):
        ...
        curHeading.setZ(curHeading.getZ() - min(0.20000000000000001, max(-0.20000000000000001, push)))
        ...
        if self.proRacer:
            driftMin = self.surfaceModifiers[self.groundType]['driftMin'] * 0.20000000000000001
            if self.skidding:
                driftMin = self.surfaceModifiers[self.groundType]['driftMin']
    ...
    def enableControls(self):
        ...
        self.accept('InputState-turnRight', self._DistributedVehicle__rightArrow)
    ...
    def stopCar(self, level):
        ...
        if hasattr(self, 'cameraTrack') and self.cameraTrack:
            self.cameraTrack.pause()
            cameraToNormal = Parallel(LerpPosInterval(camera, 0.050000000000000003, Point3(0, -33, 16), startPos = camera.getPos()), LerpFunc(base.camLens.setFov, fromData = base.camLens.getFov()[0], toData = ToontownGlobals.DefaultCameraFov, duration = 0.050000000000000003))
    ...
    def dropOnMe(self, timestamp):
        ...
        flattenMe = self.geom[0].scaleInterval(0.080000000000000002, Vec3(1, 1, 0.10000000000000001))
        unFlatten = Sequence(self.geom[0].scaleInterval(0.20000000000000001, Vec3(1, 1, 2)), self.geom[0].scaleInterval(0.10000000000000001, 1))
        anvilSquish = Parallel(Parallel(anvilDrop, shadowScale), Sequence(Wait(0.92000000000000004), flattenMe))
        anvilStretchFade = Sequence(Parallel(self.anvil.scaleInterval(0.20000000000000001, Vec3(7, 9, 5)), Func(self.lookUp)), fadeOut)
        self.gagMovie = Sequence(anvilSquish, Func(self.stopCar, 0.90000000000000002), anvilStretchFade, Wait(0.5), Parallel(unFlatten, Func(self.lookNormal)), Func(self.startCar), Func(self.hideAnvil))
        self.gagMovie.start()
    ...
    def dropOnHim(self, timestamp):
        ...
        unFlatten = Sequence(self.geom[0].scaleInterval(0.20000000000000001, Vec3(1, 1, 2)), self.geom[0].scaleInterval(0.10000000000000001, 1))
        anvilSquish = Parallel(Parallel(anvilDrop, shadowScale), Sequence(Wait(0.92000000000000004), flattenMe))
        anvilStretchFade = Sequence(Parallel(self.anvil.scaleInterval(0.20000000000000001, Vec3(10, 13, 4)), Func(self.lookUp)), fadeOut)
    ...
    def hitBanana(self):
        ...
        self.wipeOut = Sequence(Func(self.stopCar, 0.98999999999999999), spinAnim, Func(self.startCar))
        self.wipeOut.start()
