...
class LevelSuitPlannerAI(DirectObject.DirectObject):
    ...
    def __init__(self, air, level, cogCtor, battleCtor, cogSpecs, reserveCogSpecs, battleCellSpecs, battleExpAggreg = None):
        ...
        self._LevelSuitPlannerAI__genSuitInfos(self.level.getCogLevel(), self.level.getCogTrack())
    ...
    def _LevelSuitPlannerAI__genJoinChances(self, num):
        joinChances = []
    ...
    def _LevelSuitPlannerAI__genSuitInfos(self, level, track):
        if __dev__:
            pass
        1
        ...
        joinChances = self._LevelSuitPlannerAI__genJoinChances(numReserve)
    ...
    def _LevelSuitPlannerAI__genSuitObject(self, suitDict, reserve):
    ...
    def genSuits(self):
        ...
        for activeSuitInfo in self.suitInfos['activeSuits']:
            suit = self._LevelSuitPlannerAI__genSuitObject(activeSuitInfo, 0)
        ...
        for reserveSuitInfo in self.suitInfos['reserveSuits']:
            suit = self._LevelSuitPlannerAI__genSuitObject(reserveSuitInfo, 1)
            reserveSuits.append([
                suit,
    ...
    def _LevelSuitPlannerAI__suitCanJoinBattle(self, cellId):
    ...
    def requestBattle(self, suit, toonId):
        ...
        maxSuits = 4
        self.battleMgr.newBattle(cellIndex, zone, pos, suit, toonId, self._LevelSuitPlannerAI__handleRoundFinished, self._LevelSuitPlannerAI__handleBattleFinished, maxSuits)
        for otherSuit in self.battleCellId2suits[cellIndex]:
            if otherSuit is not suit:
                if self._LevelSuitPlannerAI__suitCanJoinBattle(cellIndex):
                    ...
                    return 0
            self._LevelSuitPlannerAI__suitCanJoinBattle(cellIndex)
        
        return 1

    
    def _LevelSuitPlannerAI__handleRoundFinished(self, cellId, toonIds, totalHp, deadSuits):
        totalMaxHp = 0
        level = self.level
    ...
    def _LevelSuitPlannerAI__handleBattleFinished(self, zoneId):
        pass
