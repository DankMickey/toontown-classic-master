...
class DistributedBattleBaseAI(DistributedObjectAI.DistributedObjectAI, BattleBase):
    ...
    def requestDelete(self):
        ...
        self._DistributedBattleBaseAI__removeTaskName(self.uniqueName('make-movie'))
    ...
    def _DistributedBattleBaseAI__joinSuit(self, suit):
        ...
        taskMgr.doMethodLater(toPendingTime, self._DistributedBattleBaseAI__serverJoinDone, taskName, extraArgs = (suit.doId, taskName))
    ...
    def _DistributedBattleBaseAI__makeAvPending(self, avId):
        ...
        self._DistributedBattleBaseAI__removeTaskName(self.taskName('to-pending-av-%d' % avId))
    ...
    def addToon(self, avId):
        ...
        self.accept(event, self._DistributedBattleBaseAI__handleUnexpectedExit, extraArgs = [
            avId])
        event = 'inSafezone-%s' % avId
        self.avatarExitEvents.append(event)
        self.accept(event, self._DistributedBattleBaseAI__handleSuddenExit, extraArgs = [
            avId,
            0])
    ...
    def _DistributedBattleBaseAI__joinToon(self, avId, pos):
        ...
        taskMgr.doMethodLater(toPendingTime, self._DistributedBattleBaseAI__serverJoinDone, taskName, extraArgs = (avId, taskName))
        self.taskNames.append(taskName)
    ...
    def _DistributedBattleBaseAI__makeToonRun(self, toonId, updateAttacks):
        ...
        taskMgr.doMethodLater(TOON_RUN_T, self._DistributedBattleBaseAI__serverRunDone, taskName, extraArgs = (toonId, updateAttacks, taskName))
        self.taskNames.append(taskName)

    
    def _DistributedBattleBaseAI__serverRunDone(self, toonId, updateAttacks, taskName):
    ...
    def _DistributedBattleBaseAI__handleSuddenExit(self, avId, userAborted):
        self._DistributedBattleBaseAI__removeToon(avId, userAborted = userAborted)
        if self.fsm.getCurrentState().getName() == 'PlayMovie' or self.fsm.getCurrentState().getName() == 'MakeMovie':
    ...
    def signupToon(self, toonId, x, y, z):
        ...
        if self.toonCanJoin():
            if self.addToon(toonId):
                self._DistributedBattleBaseAI__joinToon(toonId, Point3(x, y, z))
                self.d_setMembers()
    ...
    def _DistributedBattleBaseAI__addJoinResponse(self, avId, taskName, toon = 0):
        if toon == 1:
    ...
    def movieDone(self):
        ...
        if self._DistributedBattleBaseAI__allPendingActiveToonsResponded():
            ...
            self.timer.stop()
            self.timer.startCallback(TIMEOUT_PER_USER, self._DistributedBattleBaseAI__serverMovieDone)
    ...
    def requestAttack(self, track, level, av):
        ...
        if track == SOS:
            ...
            pass
        if not self.validate(toonId, track <= MAX_TRACK_INDEX, 'requestAttack: invalid track %s' % track):
            return None
        
        if level >= 0:
            pass
        if not self.validate(toonId, level <= MAX_LEVEL_INDEX, 'requestAttack: invalid level %s' % level):
            return None
    ...
    def _DistributedBattleBaseAI__requestMovie(self, timeout = 0):
        if self.adjustFsm.getCurrentState().getName() == 'Adjusting':
            ...
            if movieDelay:
                taskMgr.doMethodLater(0.80000000000000004, self._DistributedBattleBaseAI__makeMovie, self.uniqueName('make-movie'))
    ...
    def enterWaitForInput(self):
        ...
        if not self.tutorialFlag:
            self.timer.startCallback(SERVER_INPUT_TIMEOUT, self._DistributedBattleBaseAI__serverTimedOut)
    ...
    def enterPlayMovie(self):
        ...
        self.timer.startCallback(movieTime, self._DistributedBattleBaseAI__serverMovieDone)
                                ...
                                if hp > 0 and track == LURE:
                                    ...
                                    if oldBattleTrap == UBER_GAG_LEVEL_INDEX:
                                        for otherSuit in self.activeSuits:
                                            ...
                                            trapDict.has_key(otherSuit.doId)
    ...
    def enterAdjusting(self):
        ...
        self.adjustingTimer.startCallback(self._DistributedBattleBaseAI__estimateAdjustTime() + SERVER_BUFFER_TIME, self._DistributedBattleBaseAI__serverAdjustingDone)
    ...
    def enterNotAdjusting(self):
        ...
        if self.movieRequested == 1:
            if len(self.activeToons) > 0 and self._DistributedBattleBaseAI__allActiveToonsResponded():
                self._DistributedBattleBaseAI__requestMovie()
