...
class DistributedBattleAI(type(DistributedBattleBaseAI)):
    ...
    def enterFaceOff(self):
        ...
        self.timer.startCallback(timeForFaceoff, self._DistributedBattleAI__serverFaceOffDone)
    ...
    def localMovieDone(self, needUpdate, deadToons, deadSuits, lastActiveSuitDied):
        if len(self.toons) == 0:
            ...
            self.d_setMembers()
            if len(deadSuits) > 0 or lastActiveSuitDied == 0 or len(deadToons) > 0:
                self.needAdjust = 1
