...
CFSpeech = NametagGlobals.CFSpeech
CFTimeout = NametagGlobals.CFTimeout
...
def doSuitAttack(attack):
    ...
    if not battle:
        return
    resetPos = battle.getActorPosHpr(suit)
    resetHpr = battle.getActorPosHpr(suit)
...
def getSuitTrack(attack, delay = 9.9999999999999995e-007, splicedAnims = None):
    ...
    track = Sequence(Wait(delay), Func(suit.setChatAbsolute, taunt, NametagGlobals.CFSpeech | NametagGlobals.CFTimeout))
    ...
    if not battle:
        return
    origPos = battle.getActorPosHpr(suit)
    origHpr = battle.getActorPosHpr(suit)
...
def doBrainStorm(attack):
    ...
    effectColor = Vec4(0.65000000000000002, 0.79000000000000004, 0.93000000000000005, 0.84999999999999998)
...
def doFreezeAssets(attack):
    ...
    cloudPropTrack.append(LerpPosInterval(cloud, 1, pos = targetPoint))
    cloudPropTrack.append(Wait(partDelay))
    cloudPropTrack.append(ParticleInterval(snowEffect, cloud, worldRelative = 0, duration = 2.1000000000000001, cleanup = True))
    cloudPropTrack.append(Wait(0.40000000000000002))
    cloudPropTrack.append(LerpScaleInterval(cloud, 0.5, MovieUtil.PNT3_NEARZERO))
    cloudPropTrack.append(Func(MovieUtil.removeProp, cloud))
    cloudPropTrack.append(Func(battle.movie.clearRenderProp, cloud))
...
def doHotAir(attack):
    ...
    BattleParticles.setEffectTexture(flecksEffect, 'roll-o-dex', color = Vec4(0.94999999999999996, 0.94999999999999996, 0.0, 1))
    sprayDelay = 1.3
...
def doFired(attack):
    ...
    BattleParticles.setEffectTexture(baseFlameEffect, 'fire')
    BattleParticles.setEffectTexture(flameEffect, 'fire')
    BattleParticles.setEffectTexture(flecksEffect, 'roll-o-dex', color = Vec4(0.80000000000000004, 0.80000000000000004, 0.80000000000000004, 1))
    ...
    BattleParticles.setEffectTexture(flecksSmall, 'roll-o-dex', color = Vec4(0.80000000000000004, 0.80000000000000004, 0.80000000000000004, 1))
...
def doCrunch(attack):
    ...
    for i in range(0, numOfNumbers):
        ...
        next.setPos(random.random() * 0.59999999999999998 - 0.29999999999999999, random.random() * 0.59999999999999998 - 0.29999999999999999, random.random() * 0.59999999999999998 - 0.29999999999999999)
...
def doSpin(attack):
    ...
    height1 = toon.getHeight() * (random.random() * 0.20000000000000001 + 0.69999999999999996)
    height2 = toon.getHeight() * (random.random() * 0.20000000000000001 + 0.40000000000000002)
    height3 = toon.getHeight() * (random.random() * 0.20000000000000001 + 0.10000000000000001)
...
def doPeckingOrder(attack):
    ...
    for i in range(0, numBirds):
        ...
        next.setPos(random.random() * 0.59999999999999998 - 0.29999999999999999, random.random() * 0.59999999999999998 - 0.29999999999999999, random.random() * 0.59999999999999998 - 0.29999999999999999)
        if dmg > 0:
            hitPoint = Point3(random.random() * 5 - 2.5, random.random() * 2 - 1 - 6, (random.random() * 3 - 1.5) + toon.getHeight() - 0.90000000000000002)
        else:
            hitPoint = Point3(random.random() * 2 - 1, random.random() * 4 - 2 - 15, (random.random() * 4 - 2) + 2.2000000000000002)
