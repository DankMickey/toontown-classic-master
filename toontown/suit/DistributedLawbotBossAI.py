...
class DistributedLawbotBossAI(DistributedBossCogAI.DistributedBossCogAI, FSM.FSM):
    ...
    def enterRollToBattleTwo(self):
        ...
        self.barrier = self.beginBarrier('RollToBattleTwo', self.involvedToons, 50, self._DistributedLawbotBossAI__doneRollToBattleTwo)
    ...
    def enterPrepareBattleTwo(self):
        self._DistributedLawbotBossAI__makeCannons()
        self.barrier = self.beginBarrier('PrepareBattleTwo', self.involvedToons, 45, self._DistributedLawbotBossAI__donePrepareBattleTwo)
    ...
    def enterBattleTwo(self):
        if self.useCannons:
            ...
            self.barrier = self.beginBarrier('BattleTwo', self.involvedToons, ToontownGlobals.LawbotBossJuryBoxMoveTime + 1, self._DistributedLawbotBossAI__doneBattleTwo)
    ...
    def enterRollToBattleThree(self):
        self.divideToons()
        self.barrier = self.beginBarrier('RollToBattleThree', self.involvedToons, 20, self._DistributedLawbotBossAI__doneRollToBattleThree)
    ...
    def enterPrepareBattleThree(self):
        ...
        self.barrier = self.beginBarrier('PrepareBattleThree', self.involvedToons, 45, self._DistributedLawbotBossAI__donePrepareBattleThree)
    ...
    def _DistributedLawbotBossAI__saySomethingLater(self, delayTime = 15):
        ...
        taskMgr.remove(taskName)
        taskMgr.doMethodLater(delayTime, self._DistributedLawbotBossAI__saySomething, taskName)
    ...
    def enterVictory(self):
        ...
        self.barrier = self.beginBarrier('Victory', self.involvedToons, 30, self._DistributedLawbotBossAI__doneVictory)
            ...
            if not toon.hasParticularCogSummons(prefDeptIndex, cogLevel, 'single'):
                ...
                for curDeptIndex in range(len(SuitDNA.suitDepts)):
                    if not toon.hasParticularCogSummons(curDeptIndex, cogLevel, prefSummonType):
                        ...
                        break
                        continue
                    if not toon.hasParticularCogSummons(curDeptIndex, cogLevel, 'single'):
                        deptIndex = curDeptIndex
                        summonType = 'single'
                        foundOne = True
                        break
                        continue
                    if not toon.hasParticularCogSummons(curDeptIndex, cogLevel, 'building'):
                        deptIndex = curDeptIndex
                        summonType = 'building'
                        foundOne = True
                        break
                        continue
                    if not toon.hasParticularCogSummons(curDeptIndex, cogLevel, 'invasion'):
                        summonType = 'invasion'
                        deptIndex = curDeptIndex
                        foundOne = True
                        break
    ...
    def enterDefeat(self):
        self.resetBattles()
        self.barrier = self.beginBarrier('Defeat', self.involvedToons, 10, self._DistributedLawbotBossAI__doneDefeat)
