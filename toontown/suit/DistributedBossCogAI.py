...
class DistributedBossCogAI(DistributedAvatarAI.DistributedAvatarAI):
    ...
    def addToon(self, avId):
        if avId not in self.looseToons and avId not in self.involvedToons:
            ...
            self.acceptOnce(event, self._DistributedBossCogAI__handleUnexpectedExit, extraArgs = [
                avId])
    ...
    def removeToon(self, avId):
        ...
        if not self.hasToons():
            taskMgr.doMethodLater(10, self._DistributedBossCogAI__bossDone, self.uniqueName('BossDone'))
    ...
    def setState(self, state):
        ...
        if self.air:
            if state in self.keyStates:
                self.air.writeServerEvent('bossBattle', self.doId, '%s|%s|%s|%s' % (self.dept, state, self.involvedToons, self.formatReward()))
    ...
    def enterWaitForToons(self):
        self.acceptNewToons()
        self.barrier = self.beginBarrier('WaitForToons', self.involvedToons, 5, self._DistributedBossCogAI__doneWaitForToons)
    ...
    def enterElevator(self):
        ...
        self.resetBattles()
        self.barrier = self.beginBarrier('Elevator', self.involvedToons, 30, self._DistributedBossCogAI__doneElevator)
    ...
    def enterReward(self):
        self.resetBattles()
        self.barrier = self.beginBarrier('Reward', self.involvedToons, BattleBase.BUILDING_REWARD_TIMEOUT, self._DistributedBossCogAI__doneReward)
    ...
    def progressRandomValue(self, fromValue, toValue, radius = 0.20000000000000001):
