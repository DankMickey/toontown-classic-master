...
class AvatarChooser(StateData.StateData):
    ...
    def load(self, isPaid):
        ...
        self.advancedButton = DirectButton(relief=None, image=(quitHover, quitHover, quitHover), text=TTLocalizer.AvatarChoiceAdvanced, text_font=ToontownGlobals.getSignFont(), text_fg=(0.977, 0.816, 0.133, 1), text_scale=TTLocalizer.AClogoutButton, text_pos=(0, -0.035), pos=(1.17, 0, -0.914), image_scale=(1.5,1.15,1.15), image1_scale=(1.5,1.15,1.15), image2_scale=(1.55,1.18,1.18), scale=0.5, command=self._AvatarChooser__switchAvatarChooser)
        self.logoutButton.hide()
        gui.removeNode()
        gui2.removeNode()
        newGui.removeNode()
    ...
    def __switchAvatarChooser(self):
        Settings.update("enablePat3d", True)
        base.cr.pickChooseAvatar(self.avatarList)
        self.unload()
