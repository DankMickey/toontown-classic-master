...
from toontown.toonbase import TTLocalizer, ToontownGlobals, Settings
...
class NewPickAToon:
    ...
    def load(self, isPaid=1):
        ...
        bdes = loader.loadModel('phase_3/models/gui/tt_m_gui_mat_nameShop.bam')
        self.play = DirectButton(relief=None, geom=(bdes.find('**/tt_t_gui_mat_namePanelSquareUp'),
                                                    bdes.find('**/tt_t_gui_mat_namePanelSquareDown'),
                                                    bdes.find('**/tt_t_gui_mat_namePanelSquareHover'),
                                                    bdes.find('**/tt_t_gui_mat_namePanelSquareUp')),
                                 text=PLAY, text_scale=(.040,.050,.050), text_pos=(0, -0.013), scale=1.3,
                                 pos=(.8, 0, -.6), command=self.startGame, parent=self.pat2dNode,
                                 text_fg=(0.5,0.5,0.9,1), text_font=ToontownGlobals.getSignFont())
        ...
        self.basicButton = DirectButton(relief=None, geom=(bdes.find('**/tt_t_gui_mat_namePanelSquareUp'),
                                                    bdes.find('**/tt_t_gui_mat_namePanelSquareDown'),
                                                    bdes.find('**/tt_t_gui_mat_namePanelSquareHover'),
                                                    bdes.find('**/tt_t_gui_mat_namePanelSquareUp')),
                                           text=TTLocalizer.AvatarChoiceNormal, text_font=ToontownGlobals.getSignFont(),
                                           text_fg=(0.977, 0.816, 0.133, 1), text_scale=TTLocalizer.AClogoutButton,
                                           text_pos=(0, -0.035), pos=(1.17, 0, -0.974), image_scale=(1.5,1.15,1.15), 
                                           scale=0.5, command=self._NewPickAToon__switchAvatarChooser, parent=self.pat2dNode)
        ...
        self.statusText = OnscreenText(pos=(0, -.8), text='', font=ToontownGlobals.getToonFont(),
                                       fg=(1, 1, 1, 1), parent=self.pat2dNode, scale=.075, shadow=(0,0,0,1))
                                       
        self.nameYourToonButton = DirectButton(relief=None, geom=(bdes.find('**/tt_t_gui_mat_namePanelSquareUp'),
                                                    bdes.find('**/tt_t_gui_mat_namePanelSquareDown'),
                                                    bdes.find('**/tt_t_gui_mat_namePanelSquareHover'),
                                                    bdes.find('**/tt_t_gui_mat_namePanelSquareUp')),
                                               text=TTLocalizer.AvatarChoiceNameYourToon.upper().replace('\n', ' '),
                                               text_scale=(.040,.050,.050), text_pos=(0, -0.013), scale=1.3, pos=(.8, 0, -.47),
                                               command=self.__nameIt, parent=self.pat2dNode, text_fg=(0.5,0.5,0.9,1),
                                               text_font=ToontownGlobals.getSignFont())
    
    def skyTrack(self, task):
        return SkyUtil.cloudSkyTrack(task)

    def __switchAvatarChooser(self):
        Settings.update("enablePat3d", False)
        base.cr.pickChooseAvatar(self.avList)
        self.exit()
        self.unload()
    ...
    def __updateFunction(self):
        ...
        self.nameYourToonButton['text_fg'] = COLORS[self.currentSlot]
        self.play['text_fg'] = COLORS[self.currentSlot]
        self.basicButton['text_fg'] = COLORS[self.currentSlot]
