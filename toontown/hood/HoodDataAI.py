...
from toontown.safezone import DistributedRealTimeAI
...
class HoodDataAI:
    ...
    def startup(self):
        ...
        if self.realtime == True:
            zoneId = self.zoneId
            for i in xrange(self.numStreets + 2):
               realtime = DistributedRealTimeAI.DistributedRealTimeAI(self.air)
               realtime.generateWithRequired(zoneId)
               realtime.requestServerTime()
