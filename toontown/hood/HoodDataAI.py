...
class HoodDataAI:
    ...
    def createFishingPonds(self):
        ...
        for zone in self.air.zoneTable[self.canonicalHoodId]:
            ...
            if isinstance(dnaData, DNAData):
                ...
                foundFishingPonds = self.air.findFishingPonds(dnaData, zoneId, area)
                foundFishingPondGroups = self.air.findFishingPonds(dnaData, zoneId, area)
                if not foundFishingPonds:
                    return
                if not foundFishingPondGroups:
                    return
    ...
    def createFishingSpots(self):
        self.fishingSpots = []
        self.fishingSpotGroups = []
        for zone in self.air.zoneTable[self.canonicalHoodId]:
            zoneId = ZoneUtil.getTrueZoneId(zone[0], self.zoneId)
            dnaData = self.air.dnaDataMap.get(zone[0], None)
            if isinstance(dnaData, DNAData):
                area = ZoneUtil.getCanonicalZoneId(zoneId)
                (foundFishingSpots, foundFishingSpotGroups) = self.air.findFishingSpots(dnaData, zoneId)
                self.fishingSpots += foundFishingSpots
                self.fishingSpotGroups += foundFishingSpotGroups
                continue
            
            for distObj in self.fishingSpots:
                self.addDistObj(distObj)
